...
Validation on 20 September 2026: all 29 Swift tests and four engine regression tests pass; design app builds and launches, and both design and production processes remain running. Design bundle and URL identities were read back. No real process was closed or cache cleared in testing. Follow-up native inspection covered the dark overview, expanded protected work, Command-R refresh and its return to a measured result, and the light loading state. An independent reviewer found no clipping or overlap in the saved dark screenshots. Small footer contrast was improved. The screen bridge then failed during navigation to Ports, so that navigation result and later light states were not visually verified. This is branch work, not a public release.
...
The overview received a real visual pass in both appearances. Saved screenshots are in screenshots/; they are captures of the running app, not generated concepts. The design-only --light and --dark launch options make appearance checks repeatable without changing system or production preferences. Further light result-state and Ports interaction coverage remains useful after the native screen bridge recovers. The earlier memory and battery detector branches remain separate. A reviewer also noted an older receipt edge case: the engine can skip a newly protected target without updating its earlier actionable row; this preserves the process but can produce an imprecise failure receipt. Keep that as a separate engine change with a focused regression test.
